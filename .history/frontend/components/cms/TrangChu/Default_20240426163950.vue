...
const saveData = async () => {
  try {
    ...
    // Copy toàn bộ metadata ban đầu ra
    const initialMetadata = { ...oldData.Metadata };

    // Tạo biến editMetadata để sửa đổi
    const editMetadata = {
      // Đặt các giá trị mặc định hoặc giá trị đã có từ metadata ban đầu
      ...initialMetadata,
      // Thực hiện sửa đổi hoặc ghi đè các giá trị mới
      // Ví dụ:
      key: "home_page",
      label: "Trang",
      Link: "/",
      // Thêm hoặc sửa đổi các trường khác tùy ý
    };
    ...
    const patchResponse = await fetch("http://localhost:8000/page", {
      ...
      body: JSON.stringify({
        ...
        Label: "Trang",
        Link: "/",
        // Sử dụng editMetadata thay vì oldData.Metadata
