...
const fetchDataFromExternalAPI = async () => {
  try {
    // Gọi API từ bên ngoài
    const response = await fetch("http://localhost:8000/page");
    console.log(response);
    // Kiểm tra xem request có thành công hay không
    if (!response.ok) {
      throw new Error("Không thể lấy dữ liệu từ API bên ngoài.");
    }

    // Parse dữ liệu từ phản hồi của API
...
const saveData = async () => {
  // Gọi API từ bên ngoài và lấy dữ liệu
  const externalData = await fetchDataFromExternalAPI();

  // Copy mảng từ dữ liệu API
  const newDataArray = externalData.data;
  ...
  const responsePatch = await useFetch("http://localhost:8000/page", {
    ...
    body: JSON.stringify({
      ...
      Link: "/",
      Metadata: {
        data_form: newDataArray, // Chèn mảng dữ liệu mới vào Metadata
      },
    }),
  });

  // Kiểm tra và xử lý responsePatch nếu cần
};
