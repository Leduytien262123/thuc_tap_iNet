...
// Lấy dữ liệu ban đầu từ useFetch
const { data } = await useFetch("http://localhost:8000/page");
const response = data.value;

// Hiển thị thông tin Metadata của mỗi phần tử trong response
...
// Hàm xử lý để lưu dữ liệu
const saveData = async (responseData) => {
  try {
    // Step 1: Tạo bản sao của dữ liệu cũ để chỉnh sửa
    const newData = responseData.map((item) => ({ ...item }));

    // Step 2: Gửi yêu cầu PATCH để lưu dữ liệu mới
...
// Gọi hàm saveData và truyền vào response để xử lý
saveData(response);
