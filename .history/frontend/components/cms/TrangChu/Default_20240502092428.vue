...
// So sánh dữ liệu mới và dữ liệu cũ để lấy những nội dung đã thay đổi
const changedData = [];
newData.forEach((newItem, index) => {
  const oldItem = response[index];
  if (JSON.stringify(newItem) !== JSON.stringify(oldItem)) {
    changedData.push(newItem);
  }
});

// Lưu những nội dung đã thay đổi vào cơ sở dữ liệu
const saveChangedData = async () => {
  for (const item of changedData) {
    ...
    if (!patchResponse.ok) {
      throw new Error("Failed to save changed data");
    }
  }
...
// Gọi hàm để lưu những nội dung đã thay đổi
saveChangedData();
