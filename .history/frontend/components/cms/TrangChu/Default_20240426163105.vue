...
const saveData = async () => {
  ...
  const responseGet = await useFetch("http://localhost:8000/page", {
    method: "GET",
    headers: {
  ...
  // Kiểm tra xem request GET có thành công không
  ...
  const oldData = await responseGet.json();

  // Ghi đè metadata của dữ liệu cũ bằng metadata mới
  oldData.Metadata = {
    ...oldData.Metadata, // Giữ lại metadata cũ
    key: "home_page",
    Label: "Trang",
    Link: "/",
  };
  ...
  const responsePatch = await useFetch("http://localhost:8000/page", {
    ...
    body: JSON.stringify(oldData), // Gửi lại dữ liệu cũ sau khi được cập nhật
  });

  // Kiểm tra xem request PATCH có thành công không
