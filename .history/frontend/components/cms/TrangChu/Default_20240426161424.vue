...
const data_menus = response[0]?.Metadata?.data_menus ?? {};
...
const saveData = async () => {
  // Gửi request PATCH để lưu dữ liệu mới
  const responsePatch = await useFetch("http://localhost:8000/page", {
    ...
    body: JSON.stringify({
      ...
      Link: "/",
      Metadata: {
        data_form: editData.value.editForm, // Gửi dữ liệu đã chỉnh sửa
        data_menus: editData.value.editMenus, // Gửi dữ liệu đã chỉnh sửa
      },
    }),
  });

  if (responsePatch.ok) {
    // Lấy dữ liệu mới từ phản hồi của server
    const newData = await responsePatch.json();

    // Cập nhật dữ liệu trên client với dữ liệu mới từ server
    form.value = newData.Metadata.data_form;
    menus.value = newData.Metadata.data_menus;

    // Hiển thị thông báo thành công
    alert("Lưu thành công");
  } else {
    // Xử lý lỗi khi không thể lưu dữ liệu
    console.error("Lỗi khi lưu dữ liệu:", responsePatch.statusText);
