...
// Khai báo một mảng để lưu các thay đổi chỉnh sửa
const editedData = ref([]);

// Hàm để lưu dữ liệu vào local storage
const saveToLocalStorage = () => {
  localStorage.setItem("editedData", JSON.stringify(editedData.value));
};

// Hàm để lấy dữ liệu từ local storage khi trang được tải lại
const loadFromLocalStorage = () => {
  const savedData = localStorage.getItem("editedData");
  if (savedData) {
    editedData.value = JSON.parse(savedData);
  }
};

// Gọi hàm loadFromLocalStorage khi trang được tải lại
onMounted(loadFromLocalStorage);

// Hàm để lưu các thay đổi chỉnh sửa và gọi hàm lưu vào local storage
const saveData = () => {
